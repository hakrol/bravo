      ...
      JSON.stringify({
        ...
        data: {
          ...
          laborMarketStats: buildLaborMarketStats({
            ...
            workingHoursDataset: datasets.occupationWorkingHoursTimeSeries,
            occupationCode: entry.page.occupationCode,
          }),
...
function buildLaborMarketStats(options: {
  ...
  workingHoursDataset: SsbDataset;
  occupationCode: string;
}) {
  ...
  const workingHoursSeries = buildWorkingHoursSeries(
    options.workingHoursDataset,
    options.occupationCode,
  );
  ...
  const occupationLabel =
    ...
    workingHoursSeries.at(-1)?.occupationLabel ??
    options.occupationCode;
  ...
  return {
    ...
    workingHoursSeries: workingHoursSeries.map(removeOccupationLabel),
  };
}
...
function buildWorkingHoursSeries(dataset: SsbDataset, occupationCode: string) {
  const pointsByPeriod = new Map<string, {
    occupationLabel: string;
    periodCode: string;
    periodLabel: string;
  ...
  for (const row of dataset.rows) {
    ...
    setGenderValue(existing, gender.code, row.value, "hours");
    pointsByPeriod.set(period.code, existing);
  }
