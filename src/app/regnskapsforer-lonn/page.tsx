...
import {
  accountantOccupationDetailPage,
...
const accountantLabel = accountantOccupationDetailPage?.label ?? "Regnskapsfører";

export const metadata: Metadata = {
  title: `${accountantLabel} lønn`,
  description:
    `Se lønnsutvikling og gjennomsnittlig avtalt månedslønn for ${accountantLabel.toLowerCase()} basert på SSB-data.`,
};

export default async function AccountantSalaryPage() {
  const relatedPages = getRelatedOccupationDetailPages(ACCOUNTANT_OCCUPATION_CODE);
  ...
  const [series, purchasingPower, purchasingPowerSeries, latestDataset] = await Promise.all([
    getOccupationSalaryTimeSeries(ACCOUNTANT_OCCUPATION_CODE, OCCUPATION_MONTHLY_SALARY_FILTERS),
    getOccupationPurchasingPowerDetail(ACCOUNTANT_OCCUPATION_CODE, OCCUPATION_MONTHLY_SALARY_FILTERS),
    getOccupationPurchasingPowerTimeSeries(
      ACCOUNTANT_OCCUPATION_CODE,
      OCCUPATION_MONTHLY_SALARY_FILTERS,
    ),
  ...
  const currentSalary =
    overviewRowsByCode.get(ACCOUNTANT_OCCUPATION_CODE)?.salaryAll ?? latestSalaryPoint?.valueAll;
  const currentSalaryWomen =
    overviewRowsByCode.get(ACCOUNTANT_OCCUPATION_CODE)?.salaryWomen ?? latestSalaryPoint?.valueWomen;
  const currentSalaryMen =
    overviewRowsByCode.get(ACCOUNTANT_OCCUPATION_CODE)?.salaryMen ?? latestSalaryPoint?.valueMen;
    ...
    <main className="min-h-screen px-5 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8">
          ...
          <div className="space-y-8">
            <div className="space-y-3">
              ...
              <p className="max-w-3xl text-base leading-7 text-[var(--muted)]">
                ...
                />{" "}
                for {accountantLabel.toLowerCase()}. Tallene viser bruttolønn før skatt, og
                inkluderer ikke overtid eller bonus. Regnskapsførere fører regnskap, avstemmer
                tall og utarbeider rapporter og dokumentasjon for virksomheter. Mange jobber også
                med lønn, skattemelding, årsoppgjør og økonomisk rådgivning.
              </p>
            </div>
            {purchasingPower ? (
        ...
        <RelatedOccupationSalaryComparison
          currentOccupationLabel={accountantLabel}
